...
- Keep lowercase `awmate` in commands, package names, paths, URLs, and other exact identifiers
...
- Keep product naming consistent: `AWMate` in prose, `awmate` only for exact technical identifiers
...
- Avoid changing `awmate` to `AWMate` inside commands, URLs, package names, or code literals
