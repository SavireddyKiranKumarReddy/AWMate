...
import { createAWMate } from "@awmate/sdk"
...
`)
  ...
  const awmate = await createAWMate({ port: 0 })
  ...
  try {
    ...
    const session = await awmate.client.session.create()
    const result = await awmate.client.session
      .prompt({
