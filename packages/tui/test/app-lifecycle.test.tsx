...
test("app.exit prints the session epilogue after scoped cleanup", async () => {
  ...
  try {
    ...
    expect(stdout).toContain("awmate -s dummy")
  } finally {
...
test("fatal startup errors set a nonzero exit after scoped cleanup", async () => {
      ...
      return json(
        {
          ...
          data: {
            ...
            remote: "https://config.example.com/awmate.json",
          },
        },
        { status: 400 },
      )
  ...
  try {
    ...
    await task
    expect(stderr).toContain("Run `awmate auth login https://example.com` to re-authenticate.")
