...
import { AWMatePlugin } from "@awmate/core/plugin/provider/awmate"
...
const addPlugin = Effect.fn(function* (http?: HttpClient.HttpClient) {
  ...
  yield* AWMatePlugin.effect(host).pipe(
...
describe("AWMatePlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("awmate")))?.methods).toEqual([
        {
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          HttpClientResponse.fromWeb(
            ...
            Response.json({
              device_code: "device",
              user_code: "user",
              verification_uri_complete: "/console/device?user_code=user&client_id=awmate-cli",
              expires_in: 60,
              interval: 60,
            }),
          ),
      ...
      const attempt = yield* integration.connection.oauth({
        integrationID: Integration.ID.make("awmate"),
        methodID: Integration.MethodID.make("device"),
        inputs: {},
      })
      expect(attempt.url).toBe("https://opencode.ai/console/device?user_code=user&client_id=awmate-cli")
    }),
  )
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("awmate"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "awmate",
            api: {
              type: "aisdk",
      ...
      Effect.gen(function* () {
        ...
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.awmate),
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.awmate)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.awmate, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ AWMATE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.awmate)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.awmate, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ AWMATE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.awmate)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.awmate, ModelV2.ID.make("output-only"))).enabled).toBe(
          true,
        )
      }),
    ),
  ...
  it.effect("uses AWMATE_API_KEY as credentials", () =>
    withEnv({ AWMATE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.awmate)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.awmate, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ AWMATE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.awmate)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.awmate, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-awmate providers and models", () =>
    withEnv({ AWMATE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the awmate small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.awmate
