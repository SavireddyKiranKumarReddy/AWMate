...
const awmateClient = process.env.AWMATE_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (awmateClient === undefined) delete process.env.AWMATE_CLIENT
  else process.env.AWMATE_CLIENT = awmateClient
})
...
describe("resource", () => {
  ...
  test("keeps built-in attributes when env values conflict", () => {
    ...
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "awmate.client=web,service.instance.id=override,service.namespace=anomalyco"
...
test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "awmate-log-test-"))
  await using _ = {
