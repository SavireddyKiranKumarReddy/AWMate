...
export const AWMatePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "awmate",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("AWMatePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("awmate")
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("awmate", (integration) => {
        integration.name = "AWMate"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "awmate", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("awmate")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("awmate")
      ...
      const item = catalog.provider.get(ProviderV2.ID.awmate)
      if (!item) return
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("awmate")),
