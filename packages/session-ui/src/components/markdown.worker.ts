...
import { createMarkdownParser } from "@awmate/ui/context/marked-parser"
import { AWMateTheme } from "@awmate/ui/context/marked-theme"
import {
  bundledLanguages,
  createHighlighter,
  getTokenStyleObject,
...
const parser = createMarkdownParser(async (code, language) => {
  ...
  return instance.codeToHtml(code, { lang: name as BundledLanguage, theme: "AWMate", tabindex: false })
})
...
function getHighlighter() {
  return (highlighter ??= createHighlighter({ themes: [AWMateTheme], langs: [] }))
}
