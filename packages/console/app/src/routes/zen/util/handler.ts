...
export async function handler(
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://ai.awmate.nxtgensec.org/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://ai.awmate.nxtgensec.org/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://ai.awmate.nxtgensec.org/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://ai.awmate.nxtgensec.org/workspace/${authInfo.workspaceID}/members`
