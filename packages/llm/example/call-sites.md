...
Kit and Aidan want provider-specific LLM behavior to move out of awmate's AI
...
awmate's dynamic runtime should construct executable models at its app
...
- [ ] Update tests so direct route/provider tests assert route values are carried
      by executable models, and awmate/native tests assert boundary-based route
      selection.
