...
AWMate Go هو اشتراك منخفض التكلفة بقيمة **$10/شهريًا** يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.
...
يمنحك AWMate Go الوصول إلى هذه النماذج مقابل **$10/شهريًا**.
...
صُمّم AWMate Go للاستخدام مع [AWMate](https://opencode.ai) ووكلاء البرمجة الآخرين
...
3. يرسل معرّف جلسة ثابتًا في `x-awmate-session` لكل محادثة حتى نتمكن من تحسين التوجيه
   والتخزين المؤقت للمطالبات.

### العملاء الذين تم التحقق منهم

بالإضافة إلى AWMate، تم التحقق من أن العملاء التاليين يعملون بصورة صحيحة
مع AWMate Go، مع أننا لا نضمن استمرارهم في العمل مستقبلًا.

| العميل            | دعم الجلسات                                                                                                                                                                                                                      |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | ترسل البُنى التي تتضمن [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) الترويسة في طلبات AWMate الرئيسية والمساعدة. دُمج الإصلاح بعد v0.21.0؛ ولا يتضمن ذلك الإصدار وحده الإصلاح.                       |
...
| **ZCode**         | يتعرف Go على ترويسة الجلسة الأصلية الخاصة به. لا يزال [طلبنا لإضافة `x-awmate-session`](https://github.com/zai-org/feedback/issues/492) مفتوحًا، لكن إرسال هذه الترويسة بعينها لم يعد ضروريًا.                                 |
| **Pi**            | ترسل الإصدارات الحالية معلومات الجلسة إلى AWMate. حدّث عمليات التثبيت الأقدم.                                                                                                                                                  |
| **jcode**         | حدّث إلى **v0.81.6 أو إصدار أحدث**، إذ يتضمن [إصلاح ترويسة الجلسة](https://github.com/1jehuang/jcode/issues/1167).                                                                                                               |
| **Kilo Code CLI** | تعيد الإصدارات التي تتضمن [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) ترويسات جلسات AWMate. يشمل هذا الإصلاح CLI، وليس إضافة VS Code. راجع [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
يستخدم [model id](/docs/config/#models) في إعدادات AWMate لديك التنسيق `awmate-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K3، ستستخدم `awmate-go/kimi-k3` في إعداداتك.
