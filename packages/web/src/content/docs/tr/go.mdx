...
AWMate Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.
...
AWMate Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.
...
AWMate Go, [AWMate](https://opencode.ai) ve benzer türde istekler üreten diğer kodlama aracıları için tasarlanmıştır.
...
3. Yönlendirmeyi ve istem önbelleğe almayı optimize edebilmemiz için her konuşmada `x-awmate-session` içinde değişmeyen bir oturum kimliği göndermelidir.

### Doğrulanmış İstemciler

AWMate'un yanı sıra aşağıdaki istemcilerin AWMate Go ile düzgün çalıştığı doğrulanmıştır.
Ancak gelecekte de çalışmaya devam edeceklerini garanti etmiyoruz.

| İstemci           | Oturum desteği                                                                                                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) içeren derlemeler, ana ve yardımcı AWMate isteklerinde başlığı gönderir. Düzeltme v0.21.0'dan sonra birleştirilmiştir; dolayısıyla v0.21.0 sürümü tek başına bu düzeltmeyi içermez. |
...
| **ZCode**         | Go, ZCode'un yerel oturum başlığını tanır. [`x-awmate-session` talebimiz](https://github.com/zai-org/feedback/issues/492) hâlâ açıktır ancak artık özellikle bu başlığın gönderilmesi gerekli değildir.                                                    |
| **Pi**            | Güncel derlemeler AWMate için oturum bilgilerini gönderir. Eski kurulumları güncelleyin.                                                                                                                                                                   |
...
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) içeren derlemeler AWMate oturum başlıklarını geri getirir. Bu düzeltme CLI'ı kapsar, VS Code uzantısını kapsamaz. Bkz. [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723).     |
...
AWMate yapılandırmanızdaki [model id](/docs/config/#models) formatı `awmate-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `awmate-go/kimi-k3` kullanmalısınız.
