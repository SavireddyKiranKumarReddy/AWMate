...
O AWMate Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.
...
O AWMate Go lhe dá acesso a esses modelos por **US$ 10/mês**.
...
O AWMate Go foi projetado para o [AWMate](https://opencode.ai) e outros agentes de programação
...
3. Enviar um ID de sessão estável em `x-awmate-session` para cada conversa, para que possamos otimizar o roteamento e
   o cache de prompts.

### Clientes validados

Além do AWMate, os clientes a seguir foram validados para funcionar corretamente
com o AWMate Go. No entanto, não garantimos que continuarão funcionando no futuro.

| Cliente           | Suporte a sessões                                                                                                                                                                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | As builds que contêm o [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) enviam o header nas requisições principais e auxiliares do AWMate. A correção foi incorporada após a v0.21.0; essa versão por si só não a inclui.                     |
...
| **ZCode**         | O Go reconhece seu header de sessão nativo. Nossa [solicitação de `x-awmate-session`](https://github.com/zai-org/feedback/issues/492) continua aberta, mas não é mais necessário enviar especificamente esse header.                                                |
| **Pi**            | As builds atuais enviam informações de sessão para o AWMate. Atualize instalações mais antigas.                                                                                                                                                                     |
...
| **Kilo Code CLI** | As builds que contêm o [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restauram os headers de sessão do AWMate. Essa correção abrange a CLI, não a extensão do VS Code. Consulte a [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
usa o formato `awmate-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`awmate-go/kimi-k3` na sua configuração.
