...
:::note
This tool is only available when using the AWMate or AWMate Go provider, or when either the `AWMATE_ENABLE_EXA` or `AWMATE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).
...
# or
AWMATE_ENABLE_PARALLEL=1 awmate
```
