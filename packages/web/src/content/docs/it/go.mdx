...
AWMate Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.
...
AWMate Go ti dà accesso a questi modelli a **10 $/mese**.
...
AWMate Go è progettato per [AWMate](https://opencode.ai) e altri agenti di programmazione
...
3. Inviare un ID di sessione stabile in `x-awmate-session` per ogni conversazione, in modo da consentirci di ottimizzare il routing e
   il caching dei prompt.

### Client convalidati

Oltre a AWMate, è stato verificato che i seguenti client funzionino correttamente
con AWMate Go. Tuttavia, non garantiamo che continueranno a funzionare in futuro.

| Client            | Supporto delle sessioni                                                                                                                                                                                                                                                      |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Le build contenenti la [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) inviano l'header nelle richieste AWMate principali e ausiliarie. La correzione è stata unita dopo la v0.21.0; quella versione da sola non la include.                        |
...
| **ZCode**         | Go riconosce il suo header di sessione nativo. La nostra [richiesta per `x-awmate-session`](https://github.com/zai-org/feedback/issues/492) è ancora aperta, ma non è più necessario inviare proprio quell'header.                                                         |
| **Pi**            | Le build attuali inviano le informazioni di sessione per AWMate. Aggiorna le installazioni meno recenti.                                                                                                                                                                   |
...
| **Kilo Code CLI** | Le build contenenti la [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) ripristinano gli header di sessione AWMate. Questa correzione riguarda la CLI, non l'estensione VS Code. Consulta l'[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
utilizza il formato `awmate-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`awmate-go/kimi-k3` nella tua configurazione.
