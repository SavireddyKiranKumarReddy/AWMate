...
AWMate Goは、人気のオープンなコーディングモデルに安定してアクセスできる、低価格の**月額10ドルのサブスクリプション**です。
...
AWMate Goを使用すると、これらのモデルに**月額10ドル**でアクセスできます。
...
AWMate Goは、[AWMate](https://opencode.ai)および同様の種類のリクエストを生成するその他のコーディングエージェント向けに設計されています。
...
3. ルーティングとプロンプトキャッシュを最適化できるよう、会話ごとに安定したセッションIDを`x-awmate-session`で送信する。

### 動作確認済みのクライアント

AWMateに加えて、以下のクライアントがAWMate Goで正常に動作することを確認しています。
ただし、今後も動作し続けることを保証するものではありません。

| クライアント      | セッション対応                                                                                                                                                                                                                                                                |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864)を含むビルドでは、AWMateのメインリクエストと補助リクエストの両方でヘッダーが送信されます。この修正はv0.21.0より後にマージされたため、v0.21.0自体には含まれていません。                                |
...
| **ZCode**         | GoはZCode固有のセッションヘッダーを認識します。[`x-awmate-session`の追加リクエスト](https://github.com/zai-org/feedback/issues/492)は未解決ですが、この特定のヘッダーを送信する必要はなくなりました。                                                                       |
| **Pi**            | 現在のビルドはAWMateにセッション情報を送信します。古いインストールは更新してください。                                                                                                                                                                                      |
| **jcode**         | [セッションヘッダーの修正](https://github.com/1jehuang/jcode/issues/1167)を含む**v0.81.6以降**に更新してください。                                                                                                                                                            |
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752)を含むビルドでは、AWMateのセッションヘッダーが再び送信されます。この修正の対象はCLIであり、VS Code拡張機能ではありません。[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723)を参照してください。 |
...
AWMate設定の[model id](/docs/config/#models)は、`awmate-go/<model-id>`という形式を使用します。たとえば、Kimi K3の場合は、設定で`awmate-go/kimi-k3`を使用します。
