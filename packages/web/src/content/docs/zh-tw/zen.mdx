...
你可能會在使用紀錄中看到 Haiku、Nano 或 Flash 等[低成本模型](/docs/config/#models)。AWMate 使用這些模型產生工作階段標題。
...
- Ling 3.0 Flash Fin Free 在 AWMate 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Ultra Free 在 AWMate 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3.5 Lightning Free 在 AWMate 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 AWMate 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
- Muse Spark 1.3 Contributor Free 在 AWMate 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Jev 1.13 Free 在 AWMate 上限時提供。
