...
1. Run the `/connect` command in the TUI, select `AWMate Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
1. Run the `/connect` command in the TUI, select `AWMate Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
AWMate does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. AWMate refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
2. Select **SuperGrok Subscription**. AWMate opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by AWMate. AWMate polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
