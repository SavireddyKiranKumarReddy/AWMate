...
AWMate Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.
...
AWMate Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.
...
AWMate Go wurde für [AWMate](https://opencode.ai) und andere Coding-Agenten entwickelt,
...
3. für jede Unterhaltung eine stabile Sitzungs-ID in `x-awmate-session` senden, damit wir das Routing und
   Prompt-Caching optimieren können.

### Validierte Clients

Neben AWMate wurden die folgenden Clients für die ordnungsgemäße Verwendung mit
AWMate Go validiert. Wir garantieren jedoch nicht, dass sie auch in Zukunft funktionieren werden.

| Client            | Sitzungsunterstützung                                                                                                                                                                                                                                         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Builds mit [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) senden den Header bei Haupt- und zusätzlichen AWMate-Anfragen. Der Fix wurde nach v0.21.0 gemergt; diese Version allein enthält ihn nicht.                                |
...
| **ZCode**         | Go erkennt seinen nativen Sitzungs-Header. Unsere [Anfrage für `x-awmate-session`](https://github.com/zai-org/feedback/issues/492) ist weiterhin offen, aber es ist nicht mehr erforderlich, genau diesen Header zu senden.                                 |
| **Pi**            | Aktuelle Builds senden Sitzungsinformationen für AWMate. Aktualisiere ältere Installationen.                                                                                                                                                                |
...
| **Kilo Code CLI** | Builds mit [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) stellen AWMate-Sitzungs-Header wieder her. Dieser Fix gilt für die CLI, nicht für die VS-Code-Erweiterung. Siehe [Issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
Die [Modell-ID](/docs/config/#models) in deiner AWMate Config verwendet das Format `awmate-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `awmate-go/kimi-k3` in deiner Config verwenden.
