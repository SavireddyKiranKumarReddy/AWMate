...
Вы можете определить серверы MCP в своем [AWMate Config](https://ai.awmate.nxtgensec.org/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
