...
AWMate Go est un abonnement à bas coût à **10 $/mois** qui vous donne un accès fiable aux modèles de codage ouverts populaires.
...
AWMate Go vous donne accès à ces modèles pour **10 $/mois**.
...
AWMate Go est conçu pour [AWMate](https://opencode.ai) et d'autres agents de codage
...
3. Envoyer un ID de session stable dans `x-awmate-session` pour chaque conversation afin que nous puissions optimiser le routage et
   la mise en cache des prompts.

### Clients validés

Outre AWMate, le bon fonctionnement des clients suivants avec AWMate Go a été
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Les builds contenant la [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) envoient l'en-tête sur les requêtes AWMate principales et auxiliaires. Le correctif a été fusionné après la v0.21.0 ; cette version seule ne l'inclut pas.               |
...
| **ZCode**         | Go reconnaît son en-tête de session natif. Notre [demande concernant `x-awmate-session`](https://github.com/zai-org/feedback/issues/492) reste ouverte, mais il n'est plus nécessaire d'envoyer spécifiquement cet en-tête.                                             |
| **Pi**            | Les builds actuels envoient les informations de session pour AWMate. Mettez à jour les installations plus anciennes.                                                                                                                                                    |
...
| **Kilo Code CLI** | Les builds contenant la [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) rétablissent les en-têtes de session AWMate. Ce correctif concerne la CLI, pas l'extension VS Code. Consultez l'[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
L'[ID de modèle](/docs/config/#models) dans votre configuration AWMate utilise le format `awmate-go/<model-id>`. Par exemple, pour Kimi K3, vous utiliseriez `awmate-go/kimi-k3` dans votre configuration.
