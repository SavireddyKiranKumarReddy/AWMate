...
// Access verification is cached locally so relaunching the app does not block
// on Supabase. The cached grant is only trusted when it is still active, has
// not expired, and was checked recently; it is re-validated in the background
// right after the app opens, so revoked access is still caught quickly.
const ACCESS_CACHE_KEY = "awmate.desktop.access"
const ACCESS_CACHE_TTL_MS = 24 * 60 * 60 * 1000
...
function readCachedAccess(userId: string): CachedAccess | undefined {
  try {
    ...
    if (!isActiveAccess(cached.grant, Date.now())) return undefined
    if (Date.now() - cached.checkedAt > ACCESS_CACHE_TTL_MS) return undefined
    return cached
  } catch {
    return undefined
  }
...
function writeCachedAccess(userId: string, grant: AccessGrant) {
  try {
    localStorage.setItem(ACCESS_CACHE_KEY, JSON.stringify({ userId, grant, checkedAt: Date.now() }))
  } catch {}
}
...
export function DesktopAuthGate(props: ParentProps) {
  ...
  const verifyAccess = async (current: number, user: User, supabase: SupabaseClient) => {
    const grant = await loadAccessGrant(supabase, user)
    if (current !== revision) return
    if (isActiveAccess(grant)) {
      writeCachedAccess(user.id, grant!)
  ...
  const applySession = async (next: Session | null, label = "Verifying your account", skipCache = false) => {
    ...
    const user = next.user ?? (await verifySession(supabase, next))
    if (current !== revision) return

    if (!skipCache) {
      const cached = readCachedAccess(user.id)
      if (cached) {
        // Fast path: reuse the last verified grant so relaunching does not
        // block on Supabase. Re-validate in the background and correct the
        // state if access changed while the app was closed.
        setState({ kind: "active", user, grant: cached.grant })
        void verifyAccess(current, user, supabase).catch(() => {
          if (current === revision) writeCachedAccess(user.id, cached.grant)
        })
        return
      }
    }
    ...
    await verifyAccess(current, user, supabase)
  }

  // Access is only re-checked on explicit refresh (e.g. the "Check again"
  // button), not on every window focus. Once verified, access persists until
  // sign out or app restart.
  const refresh = async () => {
    if (!session) return
    await applySession(session, "Checking your AWMate access", true)
  }
