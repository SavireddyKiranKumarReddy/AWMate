...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier AWMate launch", () => {
    expect(hasExistingAppState([file("awmate.settings")])).toBe(true)
    expect(hasExistingAppState([file("awmate.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("awmate")])).toBe(true)
  })
})
