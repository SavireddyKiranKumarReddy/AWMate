  ...
  return {
    ...
    async installAWMate(name: string) {
      await runJob({ kind: "install-awmate", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installAWMate"))
        }
...
function awmateCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.awmateMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.awmateCannotRun"),
    }
  }
  return {
    distro,
