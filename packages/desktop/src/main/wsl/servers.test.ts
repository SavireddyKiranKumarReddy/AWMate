...
let releaseAWMateResolve: (() => void) | undefined
...
test("ignores stale background AWMate checks after removing a WSL server", async () => {
  persistedServers = []
  releaseAWMateResolve = undefined
  ...
  await waitFor(() => !!releaseAWMateResolve)
  await controller.removeServer("wsl:Debian")
  releaseAWMateResolve?.()
