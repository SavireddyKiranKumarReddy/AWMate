...
// `awmate.run(message, opts?)` to spawn `bun src/index.ts run ...` with
...
describe("awmate run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const result = yield* awmate.run("say hi")
        awmate.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const result = yield* awmate.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* awmate.run("think", { extraArgs: ["--thinking"] })
        awmate.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* awmate.run("think again")
        awmate.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const result = yield* awmate.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const result = yield* awmate.run("say hi", { format: "json" })
        awmate.expectExit(result, 0)

        const events = awmate.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ awmate }) =>
      Effect.gen(function* () {
        const result = yield* awmate.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const result = yield* awmate.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* awmate.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* awmate.run("request permission", { permission: { bash: "ask" } })
        awmate.expectExit(denied, 0)
        ...
        const allowed = yield* awmate.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        awmate.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* awmate.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        awmate.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, awmate }) =>
      Effect.gen(function* () {
        ...
        const server = yield* awmate.serve()

        const result = yield* awmate.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, awmate }) =>
      Effect.gen(function* () {
        const result = yield* awmate.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, awmate }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* awmate.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
