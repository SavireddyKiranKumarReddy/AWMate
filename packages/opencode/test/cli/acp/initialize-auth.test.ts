...
describe("awmate acp initialize/auth subprocess", () => {
  cliIt.live(
  ...
  cliIt.live(
    ...
    ({ awmate }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ awmate })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("awmate-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "awmate-login" })).toMatchObject({
          result: {},
        })
  ...
  cliIt.live(
    ...
    ({ awmate }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("awmate-login")
