...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ awmate }) =>
    Effect.gen(function* () {
      const result = yield* awmate.spawn(["--replay-limit", "10"])

      awmate.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ awmate }) =>
    Effect.gen(function* () {
      const result = yield* awmate.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ awmate }) =>
    Effect.gen(function* () {
      const result = yield* awmate.spawn(["--mini", "--port", "4096"])
