// Subprocess test harness for the awmate CLI. Spawns the real binary against
...
//
// Configuration flows through awmate's built-in test affordances:
...
//
// Today only `awmate.run` is fully wired. The shape supports adding more
// builders (`awmate.serve(opts)`, `awmate.acp(opts)`, `awmate.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of AWMateCli.
...
const awmateRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(awmateRoot, "src/index.ts")
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("awmate.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("awmate.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("awmate.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
    ...
    const acp = Effect.fn("awmate.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const awmate: AWMateCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, awmate })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `awmate.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "awmate") {
...
// every call site. Use this for any test that needs the awmate CLI fixture.
//
...
// resources (e.g. `awmate.serve`) without an extra `Effect.scoped` wrapper —
