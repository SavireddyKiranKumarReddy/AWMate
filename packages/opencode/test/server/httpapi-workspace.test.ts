...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-awmate-directory", directory)
  ...
  it.live("proxies remote workspace HTTP requests with sanitized forwarding", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-awmate-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-awmate-workspace")
