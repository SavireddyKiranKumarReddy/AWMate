    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          "x-awmate-directory": "/secret/path",
          "x-awmate-workspace": "internal",
        }),
      ...
      expect(forwarded?.headers["x-awmate-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-awmate-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-awmate-directory", headerDir),
        HttpClient.execute,
      )
