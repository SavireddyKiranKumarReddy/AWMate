...
describe("tool.registry", () => {
  ...
  withCodeMode.instance("exposes execute when code mode is enabled", () =>
    Effect.gen(function* () {
      ...
      const tools = yield* registry.tools({
        providerID: ProviderV2.ID.awmate,
  ...
  it.instance("loads tools from .awmate/tool (singular)", () =>
    Effect.gen(function* () {
      ...
      const awmate = path.join(test.directory, ".awmate")
      const tool = path.join(awmate, "tool")
  ...
  // came in through `plugin.list()` — `oh-my-awmate` was registering a tool
  ...
  it.instance("loads tools from .awmate/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const awmate = path.join(test.directory, ".awmate")
      const tools = path.join(awmate, "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const awmate = path.join(test.directory, ".awmate")
        const customTools = path.join(awmate, "tools")
        const plugin = path.join(awmate, "node_modules", "@awmate", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(awmate, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(awmate, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(awmate, "node_modules", "cowsay")
