    ...
    withTmp(
      async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["awmate-openai-codex-auth@1.0.0", "awmate-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
      ...
      (_tmp) =>
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(pkgs).not.toContain("awmate-openai-codex-auth@1.0.0")
            expect(pkgs).not.toContain("awmate-copilot-auth@1.0.0")
          } finally {
            install.mockRestore()
          }
        }),
