...
When `plugin` entries exist in a writable `.awmate` dir or `AWMATE_CONFIG_DIR`, AWMate installs `@awmate/plugin` into that dir and writes:
...
- `message` is required; `title` defaults to `"awmate"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- `--pure` / `AWMATE_PURE` skips external TUI plugins only.
