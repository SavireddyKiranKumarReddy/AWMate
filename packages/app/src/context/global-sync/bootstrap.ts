...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: AWMateClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("awmate-test"))
