...
import { useTheme } from "@awmate/ui/theme"
...
const featuredProviders = ["awmate", "awmate-go", "openai", "anthropic", "google", "github-copilot"]
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const isFree = (item: ReturnType<ModelState["list"]>[number]) =>
    item.provider.id === "awmate" && (!item.cost || item.cost.input === 0)
