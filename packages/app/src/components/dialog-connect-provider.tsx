import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@awmate/client/promise"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "awmate-go") return language.t("dialog.provider.awmateGo.tagline")
    return undefined
  }
...
function ProviderPickerV2(props: {
  ...
  const featured = ["awmate", "awmate-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "awmate" || provider.id === "awmate-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "awmate"
                                ? "dialog.provider.awmate.tagline"
                                : "dialog.provider.awmateGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "awmate" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "awmate-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "awmate"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.awmateZen.line1")}</div>
              <div>{language.t("provider.connect.awmateZen.line2")}</div>
              <div>
                {language.t("provider.connect.awmateZen.visit.prefix")}
                <ExternalLink
                  href="https://opencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.awmateZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.awmateZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "awmate"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.awmateZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.awmateZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.awmateZen.visit.prefix")}
