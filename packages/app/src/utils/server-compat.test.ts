...
describe("createCompatibleApi", () => {
  ...
  test("routes V1 archive through the legacy session update", async () => {
    ...
    expect(requests[0]!.headers.get("x-awmate-directory")).toBe("%2Frepo")
  ...
  */
  ...
  test("disposes the V1 instance after connecting a provider", async () => {
    ...
    expect(requests[1]!.headers.get("x-awmate-directory")).toBe("%2Frepo")
    expect(requests[2]!.headers.get("x-awmate-directory")).toBeNull()
  })
