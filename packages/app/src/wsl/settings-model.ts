...
export function wslAWMateAction(check?: WslAWMateCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installAWMate"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateAWMate"
}
...
export function addServerViewModel(input: {
  ...
  const awmateCheck = selectedDistro ? (state?.awmateChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateAWMate" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.awmateMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  awmateCheck: WslAWMateCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  if (!addServerAWMateReady(input.awmateCheck)) {
    const update = !!input.awmateCheck?.resolvedPath && input.awmateCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingAWMate
        ? { key: "wsl.onboarding.updatingAWMate" }
        : update
          ? { key: "wsl.onboarding.updateAWMate" }
...
function addServerAWMateReady(check: WslAWMateCheck | null) {
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.awmateChecks[item.name]) return [`awmate:${item.name}`]
    return []
  })
