...
function ConnectionGate(props: ParentProps<{ disableHealthCheck?: boolean; startup?: Promise<void> }>) {
  ...
  // Performs a repeated health check with a long grace period for non-http
  // connections (the local sidecar can take a while to boot), otherwise fails
  // instantly. Polls with a delay instead of hammering a server that is not up
  // yet, and only falls through to the error screen once the server has had a
  ...
  const [startupHealthCheck, healthCheckActions] = createResource(() =>
    props.disableHealthCheck
      ...
      : Effect.gen(function* () {
          ...
          Effect.timeoutOrElse({ duration: "60 seconds", orElse: () => Effect.succeed(false) }),
  ...
  return (
    <>
      ...
      <Show when={loading()}>
        <div class="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-3 bg-background-base">
          <div class="h-5 w-5 animate-spin rounded-full border-2 border-border-weak-base border-t-text-strong" />
          <p class="text-13-regular text-text-weak">{language.t("app.server.connecting")}</p>
        </div>
      </Show>
    </>
  )
