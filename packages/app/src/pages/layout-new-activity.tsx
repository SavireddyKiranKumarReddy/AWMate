...
const ActivityItem = (props: { part: ToolPart }) => (
  <div class="relative border-l border-v2-border-border-muted pb-4 pl-4 last:pb-0">
    <span
      classList={{
    ...
    />
    <div class="min-w-0 text-[13px] text-v2-text-text-base [font-weight:530]">{activityTitle(props.part)}</div>
    <Show when={activityDetail(props.part)}>
      {(detail) => (
        <div class="mt-0.5 line-clamp-2 break-all font-mono text-[11px] leading-4 text-v2-text-text-faint">
          {detail()}
        </div>
      )}
    </Show>
...
export function NewLayoutActivity() {
  ...
  return (
    ...
    >
      ...
      <div ref={scrollRef} class="min-h-0 flex-1 overflow-y-auto px-5 py-5" data-scrollable>
        <Show
          when={target()}
          fallback={
