...
import { currentSession, mockAWMateServer } from "../utils/mock-server"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#awmate-titlebar-right")
...
async function setup(page: Page, events?: () => EventPayload[]) {
  await mockAWMateServer(page, {
    ...
    provider: {
      ...
      connected: ["awmate"],
      default: { providerID: "awmate", modelID: "claude-opus-4-6" },
    },
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "awmate", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "awmate.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("awmate.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
