...
- The **PTY Environment** is a server concern rather than a Core PTY concern. PTY creation merges caller values, then the host overlay, then Core-forced terminal invariants such as `TERM` and `AWMATE_TERMINAL`.
...
- Ambient project instruction discovery honors `AWMATE_DISABLE_PROJECT_CONFIG`; global instructions remain eligible.
