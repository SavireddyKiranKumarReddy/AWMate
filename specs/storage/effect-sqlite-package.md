...
`packages/opencode` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No awmate paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- AWMate-specific path/channel selection stays in `packages/opencode`.

## AWMate Adoption Notes

These are not package requirements, but they matter for the later awmate adoption PR.

The current `packages/opencode/src/storage/db.ts` has two non-obvious semantics that the awmate wrapper must preserve when it consumes this adapter:
...
1. Add `@awmate/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@awmate/effect-drizzle-sqlite` as a dependency of `packages/opencode`.
5. Port `packages/opencode/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus awmate-specific transaction/post-commit context.
