...
- Do not replace private `packages/opencode` imports with `@awmate/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@awmate/tui`.
...
Status: Completed. The private `@awmate/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@awmate/tui`.
...
- The package has no dependency on `awmate`, `@awmate/cli`, or
  `@awmate/core`.
...
primitives now live in `@awmate/tui`. The legacy host consumes explicit
...
and build inputs through `@awmate/tui/runtime`. Movable app, component,
...
`@awmate/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@awmate/tui`. Configured references resolve through the new
...
`@awmate/tui`. The legacy host injects a narrow plugin host that retains
...
public `@awmate/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@awmate/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@awmate/core`, or either
  executable package.
...
rg '@awmate/core|packages/opencode|packages/cli' packages/tui
